...
with st.expander("🔐 Tokens (optional)"):
    hf_token = st.text_input("Hugging Face Token (optional)", type="password")
    ibm_api_key = st.text_input("IBM API Key (optional)", type="password")
    ibm_url = st.text_input("IBM API URL / Endpoint (optional)")
...
st.markdown("**OR** add structured drugs below (overrides what is parsed from text).")
...
if st.button("Analyze"):
    payload = {
        ...
        "ibm_api_key": ibm_api_key or None,
        "ibm_url": ibm_url or None
    }
    try:
